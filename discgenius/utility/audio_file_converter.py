...
logger = logging.getLogger("utility.audio_file_converter")


def convert_wav_to_mp3(config, input_path, output_path):
    logger.info("INFO - Converting mp3 to wav...")
...
def convert_result_to_mp3(config, mix_name):
    logger.info("INFO - Converting wav to mp3...")
    if '.wav' in mix_name:
        ...
        logger.info("INFO - File was converted and is located at '%s'" % mp3_path)
        return mix_mp3
    return
